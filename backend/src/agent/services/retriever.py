...
try:
    # Prefer runtime implementation under `agent.graph.agents.retriever` used
    # by the packaged runtime; fall back to other import paths if necessary.
    from agent.graph.agents.retriever import retrieve_citations  # type: ignore
except Exception:
    try:
